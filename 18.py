...
if uploaded_file is not None:
    ...
    # Target column selection
        ...
        if target_column:
            if df[target_column].dtype == 'object' or df[target_column].nunique() < 20:
                ...
                # Only plot boxplot for numeric target column
        ...
        # Step 0: Initialize df in session state if not already
    if "clean_df" not in st.session_state:
        st.session_state.clean_df = df.copy()  # use original df once

    df = st.session_state.clean_df  # always work on this
    ...
    if st.button("Drop Outliers"):
        if outlier_column not in df.columns:
            st.warning("⚠️ Please detect outliers first.")
        else:
    ...
    # Step 3: Drop datetime and object columns from X (non-numeric)
    X = X.select_dtypes(include=['int64', 'float64'])  # keep only numeric

    # Step 4: Combine and drop rows with missing values
    combined = pd.concat([X, y], axis=1)
    combined = combined.dropna()  # Drop rows with any missing values

    # Step 5: Re-split X and y after cleaning
    ...
    if X.shape[0] > 0 and y.shape[0] > 0 and not X.isnull().values.any() and not y.isnull().values.any():
        ...
        # Define and fit XGBoost model based on problem type
        if problem_type in ['binary', 'multiclass']:
        ...
        # Merge feature importance from all methods
    ...
    with st.expander("Smart Visualizer: All-in-One Dataset View"):
        ...
        # === Boxplots for Numeric vs Categorical (if both types exist) ===
        ...
        with col5:
            # === Scatterplots for numeric pairs (just 2 pairs for speed) ===
            if len(numeric_cols) >= 2:
    ...
    if len(numeric_cols) >= 2:
        ...
        # 3. Elbow Method (Optional Visualization)
        ...
        # 4. User-select or default number of clusters
    ...
    # Step 6: English Insight Generator
    ...
    #  Assuming you have your dataset as df
    # Example:
    # df = your_uploaded_dataframe

    # Drop non-numeric columns first because correlation requires numeric values
    ...
    if numeric_df.shape[1] < 2:
        ...
        # 2. Create upper triangle matrix of correlations
        upper_tri = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))

        # 3. Find features with correlation greater than 0.9
        ...
        if to_drop:
            ...
            # 4. Visualize correlation matrix (optional but cool)
            ...
            # 5. Detailed Correlation Pairs (Extra Step - Unique Part!)
